...
public class ArmBuilderWeapon extends Weapon{

    TextureRegion armRegion;
    TextureRegion armBaseRegion;
    public float armLength, armExtension, armBaseExtension, armBaseLength, posx, posy, scl1, mag1, scl2, mag2, flyingLayer;
    public boolean side = false;

    public Vec2 armStartingOffset = new Vec2();

    public ArmBuilderWeapon(){
        super();
    }

    public ArmBuilderWeapon(String name){
        super(name);
        rotate = true;
        noAttack = true;
        predictTarget = false;
    ...
    public void drawArm(float x, float y, float offsetX, float offsetY, Unit unit, float baseZ) {
        ...
        if(unit.activelyBuilding()){
            unit.drawBuildingBeam(x + offsetX, y + offsetY);
        }
    ...
    public void draw(Unit unit, WeaponMount mount) {
        ...
        boolean isBusy = unit.activelyBuilding();
        float targetExt = this.posy;
        if(isBusy){
            float len = Mathf.dst(px, py, mount.aimX, mount.aimY);
            targetExt = Math.min(len, armLength + armBaseLength);
        }
