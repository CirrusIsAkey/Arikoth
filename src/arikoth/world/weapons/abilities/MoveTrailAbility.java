package arikoth.world.weapons.abilities;
...
//credits to @andromeda, check out their mod psammos
...
public class MoveTrailAbility extends Ability {
    ...
    public MoveTrailAbility(float x, float y, boolean mirror, float width, int length, Color color, boolean teamColor){
        super();
        this.x = x;
        this.y = y;
        this.mirror = mirror;
    ...
    public void draw(Unit unit){
        ...
        trail.draw(colAlt, width * sizeMult);
        Draw.color(colAlt);
        Fill.circle(Tmp.v1.x + unit.x, Tmp.v1.y + unit.y, width * sizeMult);

        if(mirror) {
            secondTrail.draw(colAlt, width * sizeMult);
            Draw.color(colAlt);
