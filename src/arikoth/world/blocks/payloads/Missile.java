...
package arikoth.world.blocks.payloads;
...
import arc.struct.*;
import arc.util.*;
import arikoth.world.presets.stat.ArikothStat;
import arikoth.world.presets.stat.ArikothStatValues;
...
public class Missile extends Block{
    ...
    public void setStats(){
        ...
        if(user != null && bullet != null){
            stats.add(ArikothStat.used, ArikothStatValues.ammo(ObjectMap.of(user, bullet)));
        }
    }
            ...
            if(explosionBullet != null){
                for(int i = 0; i < explosions; i++){
                    Time.run(Mathf.random(maxDelay), () -> {
                        ArikothMathf.randomCirclePoint(Tmp.v1, explosionArea);
