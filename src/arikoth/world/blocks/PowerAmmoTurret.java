...
public class PowerAmmoTurret extends PowerTurret {
    public Seq<BulletType> shootTypes = new Seq<>();
    public Seq<String> shootTypeNames = new Seq<>();

    public PowerAmmoTurret(String name) {
        super(name);

        saveConfig = true;
        copyConfig = true;

        config(Integer.class, (PowerAmmoTurretBuild e, Integer idx) -> {
            e.current = idx;
        });
    }
    ...
    public class PowerAmmoTurretBuild extends PowerTurretBuild {
        public int current;
